...
ClapTrap::ClapTrap(std::string name) : _name(name)
{
	...
	_energyPoints = 10;
	_attackDamage = 0;
	std::cout << GREEN << "ClapTrap " << ClapTrap::_name << " was brought into the world" << RESET << std::endl;
}
...
void	ClapTrap::attack(const std::string& target)
{
	if (_energyPoints > 0 && _hitPoints > 0)
	{
		...
		std::cout << "ClapTrap " << _name << " doesn't have energy left to attack " << target << std::endl;
